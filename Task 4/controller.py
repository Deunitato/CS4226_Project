...
class Controller(EventMixin):
    ...
    def _handle_PacketIn (self, event):    
        ...
        log.info("packet: dpid: %s, src: %s, dst: %s, port: %s" % (dpid, src_mac, dest_mac, src_port))
        ...
        def forward(message = None):
            ...
            if src_mac not in self.table[dpid]: 
                log.debug("(Adding entry into controller table)S%s: Mac entry %s @Port %s\n: ", str(dpid), str(src_mac), str(src_port)) 
    ...
    def clear_table(self):
        log.debug("Local Mac to Port Table Dropped")
        new_table = {}
