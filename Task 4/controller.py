...
class ControllerPolicy():
	...
	def parse(self):
		...
		f = open("policy.in", "r") # Ensure that this file is in the same location as where u run the pox
		policies = f.read().split()
		log.info(policies)
		num_firewall = int(policies[0])
		num_premium = int(policies[1])
		firewall_policies = policies[2:2 + num_firewall]
		premium_policies = policies[num_firewall + 2 :]
		log.debug("\nnum of firewall: " + str(num_firewall))
		log.debug("\nPolicies: " + str(firewall_policies))
		log.debug("\n\nnum of pre: " + str(num_premium))
		log.debug("\nPre: " + str(premium_policies))
...
class Controller(EventMixin):
	...
	def _handle_ConnectionUp(self, event):
        ...
        def sendFirewallPolicy(connection, policy):
			...
			block = of.ofp_match(dl_type = 0x0800, nw_proto = 6)
			...
			log.debug("\n\n\nFirewall on %s configured " + block.show() + "\n\n", dpid)
